...
import ClassList from "./components/teacher/ClassList.jsx";
...
const App = () => {
  ...
  const ProtectedRoute = ({ children, allowedRoles }) => {
    ...
    if (!user || !storedToken) {
      // Kiểm tra cả user sau parse và token
    ...
    // Log để debug
    console.log(
      `[ProtectedRoute] User role: ${currentUserRole}, Allowed roles: ${allowedRoles}`
    );

    // Kiểm tra vai trò dựa trên user.role
    if (allowedRoles && !allowedRoles.includes(currentUserRole)) {
      console.log(
        `[ProtectedRoute] Role mismatch (${currentUserRole} not in ${allowedRoles}), redirecting.`
      );
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Router future={{ v7_startTransition: true }}>
        ...
        {/* Route riêng cho TeacherDashboard */}
